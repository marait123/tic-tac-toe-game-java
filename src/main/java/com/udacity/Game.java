...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        if(!win){
            // loop secondary diagonal
            win=true;
            for (int i = 1; i < grid.length; i++) {
                win_character=grid[i][2-i];
                if(grid[i][2-i] == '-' || grid[i][2-i]!=grid[i-1][3-i]){
                    win=false;
                    break;
                }
            }
